  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
      ...
      for (int t = 1; t < N; t++)
      {
          ...
          // actuators use previous state to compensate the latency
          if (t > 1)
          {
              a = vars[a_start + t - 2];
          ...
          //use 3-order polyfit in main, therefore use 3-prder polyfit to construct f0, psi0
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = 6 * N + 2 * (N - 1);// 6 states, 2 actuators and N timesteps
  ...
  // TODO: Set lower and upper limits for variables.
    for (int i = 0; i < delta_start; i++) // non-actuators bounds
    {
        vars_lowerbound[i] = -1.0e20;
        vars_upperbound[i] = 1.0e20;
    }
    
    // The upper and lower limits of delta are set to be -25 and 25 degrees
